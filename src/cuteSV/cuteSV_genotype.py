...
def cal_PGL(rnames, vnames, hap1_prob):
    ...
    for r in rnames:
        ...
        gls[0] *= read_like(0, gts[0], p)
        gls[1] *= read_like(0, gts[1], p)
        gls[2] *= read_like(0, gts[2], p)
        gls[3] *= read_like(0, gts[3], p)
        gls[4] *= read_like(0, gts[4], p)
    for r in vnames:
        p = hap1_prob[r] if hap1_prob.get(r) else 0.5
        gls[0] *= read_like(1, gts[0], p)
        gls[1] *= read_like(1, gts[1], p)
        gls[2] *= read_like(1, gts[2], p)
        gls[3] *= read_like(1, gts[3], p)
        gls[4] *= read_like(1, gts[4], p)
...
def duipai(
    ...
    for i in svs_list:
        ...
        if len(cover) == len(cover2_dict[idx]):
            flag += 1
        if len(cover - cover2_dict[idx]) == 0:
            flag += 1
        if len(cover2_dict[idx] - cover) == 0:
            flag += 1
        if flag != 3:
            print(idx)
            print(cover)
            ...
            correct_cover += 1
        flag = 0
        if len(overlap) == len(overlap2_dict[idx]):
            flag += 1
        if len(overlap - overlap2_dict[idx]) == 0:
            flag += 1
        if len(overlap2_dict[idx] - overlap) == 0:
            flag += 1
        if flag != 3:
            print(idx)
            print(overlap)
            ...
            lines.append(
                (
                    ...
                    "{CHR}\t{POS}\t{ID}\t{REF}\t{ALT}\t{QUAL}\t{PASS}\t{INFO}\t{FORMAT}\t{GT}:{DR}:{RE}:{PL}:{GQ}\n".format(
                        ...
                        REF=(
                            ref_chrom[max(int(i[2]) - 1, 0)]
                            if i[1] == "INS"
                            else ref_chrom[
                                max(int(i[2]) - 1, 0) : int(i[2]) - int(i[3])
                            ]
                        ),
                        ALT="%s"
                        % (
                            ref_chrom[max(int(i[2]) - 1, 0)] + i[13]
                            if i[1] == "INS"
                            else ref_chrom[max(int(i[2]) - 1, 0)]
                        ),
                        INFO=info_list,
