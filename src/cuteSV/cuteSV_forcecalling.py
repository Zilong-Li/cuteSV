                ...
                if var_type == "DEL" or var_type == "DUP":
                    for seq in sigs:
                        if chrom not in var_dict:
                            var_dict[chrom] = []
                        var_dict[chrom].append(
                            [seq[-1], int(seq[0]), int(seq[1]), seq[2]]
                        )
                elif var_type == "INS":
                    for seq in sigs:
                        if chrom not in var_dict:
                        ...
                        cigar = "<INS>"
                        var_dict[chrom].append(
                            [seq[-1], int(seq[0]), int(seq[1]), seq[2], cigar]
                        )
                elif var_type == "INV":
                    for seq in sigs:
                        if chrom not in var_dict:
                            var_dict[chrom] = []
                        var_dict[chrom].append(
                            [seq[-1], int(seq[1]), int(seq[2]), seq[3]]
                        )
                else:
                    for seq in sigs:
                        chrom1 = seq[-1]
...
def find_in_list(var_type, var_list, bias, pos, sv_end):
    ...
    if right > 0 and pos - var_list[right - 1][1] <= bias:  ##
        for i in range(right - 1, -1, -1):
            ...
            if i > 0 and (
                var_list[i][1] - var_list[i - 1][1] > bias
                or pos - var_list[i - 1][1] > bias
            ):
                break
    if var_list[right][1] - pos <= bias:
        for i in range(right, len(var_list)):
            if check_same_variant(
                var_type, var_list[i][2], sv_end, bias
            ):  # if abs(var_list[i][2] - sv_end) < 1000:
                read_id_list.add(var_list[i][3])
                search_end = var_list[i][1]
            if i < len(var_list) - 1 and (
                var_list[i + 1][1] - var_list[i][1] > bias
                or var_list[i + 1][1] - pos > bias
            ):
                break
    if search_start == -1:
        search_start = pos
...
def find_in_indel_list(
    var_type, var_list, bias_origin, pos, sv_end, threshold_gloab, multi_allele
):
    ...
    if right > 0 and pos - var_list[right - 1][1] <= bias:
        for i in range(right - 1, -1, -1):
            candidates.append(
                var_list[i]
            )  # [chrom, start, len, read_id] or [chrom, start, len, read_id, seq]
            if i > 0 and (
                var_list[i][1] - var_list[i - 1][1] > bias
                or pos - var_list[i - 1][1] > 2 * bias
            ):
                break
    if var_list[right][1] - pos <= bias:
        for i in range(right, len(var_list)):
            ...
            if i + 1 < len(read_tag[read_id]):
                ...
                if var_type == "DEL":
                    read_tag2SortedList.append(
                        [
                            read_tag[read_id][i][0],
                            int(
                                (read_tag[read_id][i][1] + read_tag[read_id][j][1]) / 2
                            ),
                            read_tag[read_id][i][2] + read_tag[read_id][j][2],
                            read_tag[read_id][i][3],
                        ]
                    )
                else:
                    read_tag2SortedList.append(
                    ...
                    if var_type == "DEL":
                        read_tag2SortedList.append(
                            [
                                ...
                                int(
                                    (
                                        read_tag[read_id][i][1]
                                        + read_tag[read_id][j][1]
                                        + read_tag[read_id][k][1]
                                    )
                                    / 3
                                ),
                                read_tag[read_id][i][2]
                                + read_tag[read_id][j][2]
                                + read_tag[read_id][k][2],
                                read_tag[read_id][i][3],
                            ]
                        )
                    else:
    ...
    if var_type == "DEL":
        ...
        for i in read_tag2SortedList[1:]:
            ...
            last_len = (last_len * (len(allele_collect[-1][0]) - 1) + i[2]) / len(
                allele_collect[-1][0]
            )
            cur_bias = last_len * threshold_gloab
        if pos == debug_pos:
        ...
        if multi_allele:
            ...
            if len(bimodal_data) > 1 and final_alleles[1][0] != final_alleles[1][-1]:
                model = KMeans(
                    n_clusters=2,
                    init=np.array(
                        [int(len(bimodal_data) / 4), int(len(bimodal_data) / 4 * 3)]
                    ).reshape(-1, 1),
                    n_init=1,
                )
                # try:
                ...
                delta1 = (
                    math.ceil((len(labels) - cate + 1) / 8)
                    if len(labels) - cate >= 3
                    else 0
                )
                min_alleles = [
                    final_alleles[1][delta0],
                    final_alleles[1][cate + delta1],
                ]
                max_alleles = [
                    final_alleles[1][cate - delta0 - 1],
                    final_alleles[1][len(labels) - delta1 - 1],
                ]
                final_alleles_filter = [[], [], [], []]
                if abs(max_alleles[0] - max_alleles[1]) >= max(
                    3
                    * max(
                        max_alleles[0] - min_alleles[0], max_alleles[1] - min_alleles[1]
                    ),
                    6,
                ):
                    allele0 = np.mean(final_alleles[1][delta0 : (cate - delta0)])
                    allele1 = (
                        np.mean(final_alleles[1][cate + delta1 :])
                        if delta1 == 0
                        else np.mean(final_alleles[1][cate + delta1 : -delta1])
                    )
                    if min(allele0, sv_end) / max(allele0, sv_end) >= min(
                        allele1, sv_end
                    ) / max(allele1, sv_end):  # choose front
                        if (
                            min(min_alleles[0], sv_end) / max(min_alleles[0], sv_end)
                            > 0.9
                            and min(max_alleles[0], sv_end)
                            / max(max_alleles[0], sv_end)
                            > 0.9
                        ):
                            if cate >= max(3, len(labels) / 5):
                                for i in range(cate):
                                    ...
                                    for j in [0, 1, 3]:
                                        final_alleles_filter[j].append(
                                            final_alleles[j][i]
                                        )
                    elif (
                        min(min_alleles[1], sv_end) / max(min_alleles[1], sv_end) > 0.9
                        and min(max_alleles[1], sv_end) / max(max_alleles[1], sv_end)
                        > 0.9
                    ):
    ...
    if var_type == "INS":
        ...
        if multi_allele:
            ...
            if len(bimodal_data) > 1 and final_alleles[1][0] != final_alleles[1][-1]:
                ...
                delta1 = (
                    math.ceil((len(labels) - cate) / 8)
                    if len(labels) - cate >= 5
                    else 0
                )
                min_alleles = [
                    final_alleles[1][delta0],
...
def force_calling_chrom(
    ...
    for record in vcf_reader.fetch():
        sv_type, chrom, sv_chr2, pos, sv_end, sv_len, sv_strand, svid, ref, alts = (
            parse_record(record)
        )
        ...
        if chrom not in svs_tobe_genotyped:
            ...
            svs_pre[chrom] = dict()
        svs_tobe_genotyped[chrom].append(
...
def solve_fc(
    ...
    for sv_type in ["DEL", "DUP", "INS", "INV", "TRA"]:
        sv_dict[sv_type] = parse_sigs_chrom(
            sv_type, temporary_dir, chrom_list, sigs_index
        )
        ...
        for i in range(len(svs_dict[chrom])):
            ...
            if sv_type == "INS" or sv_type == "DEL":
                ...
                if chrom in svs_multi and sv_start in svs_multi[chrom]:
                    ...
                    multi_allele = False
                read_id_list, search_start, search_end, CIPOS, CILEN = (
                    find_in_indel_list(
                        sv_type,
                        search_id_list,
                        sigs_bias,
                        sv_start,
                        sv_len,
                        threshold_gloab_dict[sv_type],
                        multi_allele,
                    )
                )
            else:
                sv_temp = sv_type
                sigs_bias = max_cluster_bias_dict[
                    sv_type if sv_type != "BND" else "TRA"
                ]
                if sv_len / 2 > sigs_bias:
                    sigs_bias = sv_len / 2
                # read_id_list, max_cluster_bias = find_in_list(sv_type, search_id_list, sigs_bias, sv_start, sv_end)
                read_id_list, search_start, search_end = find_in_list(
                    sv_type, search_id_list, sigs_bias, sv_start, sv_end
                )
                CIPOS = "."
                CILEN = "."
            max_cluster_bias = max(
                abs(sv_start - search_start), abs(sv_start - search_end)
            )
            max_cluster_bias = max(read_range, max_cluster_bias)
            if sv_type == "INS" or sv_type == "TRA" or sv_type == "BND":
                search_list.append(
                    (max(sv_start - max_cluster_bias, 0), sv_start + max_cluster_bias)
                )
            elif sv_type == "DEL":
                if read_range < 500:
                    search_list.append(
                        (
                            max(sv_start - max_cluster_bias, 0),
                            sv_start + max_cluster_bias,
                        )
                    )
                else:
                    search_list.append(
                        (
                            max(sv_start + abs(sv_len) / 5, 0),
                            sv_start + abs(sv_len) - abs(sv_len) / 5,
                        )
                    )
            elif sv_type == "INV":
