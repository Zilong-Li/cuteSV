    ...
    ********************************************************************************************
    ...
    if action:
        candidate_single_SV_gt = call_gt(
            path, chr, candidate_single_SV, max_cluster_bias, "DEL", sigs_index
        )
...
def resolution_INS(
    ...
    ********************************************************************************************
    ...
    if action:
        candidate_single_SV_gt = call_gt(
            path, chr, candidate_single_SV, 1000, "INS", sigs_index
        )  # max_cluster_bias
...
def call_gt(
    temporary_dir, chr, candidate_single_SV, max_cluster_bias, svtype, sigs_index
):
    ...
    for item in candidate_single_SV:
        svs_list.append(
            (max(item[7] - max_cluster_bias, 0), item[7] + max_cluster_bias)
        )
